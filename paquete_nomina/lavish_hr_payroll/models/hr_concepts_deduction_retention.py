...
class HrTypeTaxRetention(models.Model):
...
class HrConceptsDeductionRetention(models.Model):
    ...
    def _exec_python_code_base(self, localdict):
        try:
            safe_eval(self.base, localdict, mode='exec')
    ...
    def _exec_python_code_calculation(self, localdict):
        try:
            safe_eval(self.calculation, localdict, mode='exec')
            return localdict.get('result', 0.0)
        except Exception as e:
