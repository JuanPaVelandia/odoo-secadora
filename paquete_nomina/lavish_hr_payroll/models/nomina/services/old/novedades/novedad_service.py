...
class NovedadService:
    ...
    def get_novedades(self):
        ...
        Respeta la configuración de aprobación de novedades de la compañía.

        Returns:
            recordset de hr.novelties.different.concepts
        """
        if self.batch_ctx:
            return self.batch_ctx.get_employee_novelties(self.employee_id)

        # Verificar configuración de aprobación en la compañía
        company = self.payslip.company_id
        requires_approval = company.novelty_approval_required if hasattr(company, 'novelty_approval_required') else False
        ...
        if requires_approval:
            ...
            # Sin aprobación requerida: draft, approved y done
