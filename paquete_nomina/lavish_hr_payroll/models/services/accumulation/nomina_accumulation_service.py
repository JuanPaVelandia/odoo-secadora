...
class NominaAccumulationService:
    ...
    def _get_category_line_map(self, domain):
        line_rows = self.env['hr.payslip.line'].search_read(
            domain,
            ['id', 'slip_id', 'category_id', 'salary_rule_id', 'leave_id']
        )
        ...
        slip_contract_map = {
            slip.id: slip.contract_id.id
            for slip in self.env['hr.payslip'].browse(slip_ids)
            if slip.contract_id
        }
        ...
        for row in line_rows:
            slip_ref = row.get('slip_id')
            contract_id = slip_contract_map.get(slip_ref[0]) if slip_ref else False
            category_ref = row.get('category_id')
            rule_ref = row.get('salary_rule_id')
            if not contract_id or not category_ref or not rule_ref:
                continue

            key = (contract_id, category_ref[0], rule_ref[0])
            info = line_map[key]
    ...
    def _get_concept_line_map(self, domain):
        ...
        for row in line_rows:
            ...
            contract_id = slip_contract_map.get(slip_ref[0]) if slip_ref else False
            rule_ref = row.get('salary_rule_id')
            if not contract_id or not rule_ref:
                continue
            line_map[(contract_id, rule_ref[0])].append(row['id'])
    ...
    def get_categories_accumulated(
        ...
        query = """
                SELECT 
                    HP.contract_id,
                    HSR.category_id AS category_id,
                    HPL.salary_rule_id,
        ...
        if category_ids:
            # hr.payslip.line.category_id es related y no esta almacenado: usar hr_salary_rule.category_id
            query += " AND HSR.category_id = ANY(%(category_ids)s)"
            params['category_ids'] = category_ids

        query += """
            GROUP BY HP.contract_id, HSR.category_id, HPL.salary_rule_id
        """
    ...
    def get_concepts_accumulated(
        ...
        query += """
            GROUP BY HP.contract_id, HPL.salary_rule_id
        """
