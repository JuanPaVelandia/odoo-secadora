...
class HrNoveltyDifferentConcepts(models.Model):
    ...
    def action_cancel(self):
        """Cancelar novedad. Si está enlazada a una nómina en borrador, la desvincula;
        si la nómina ya está validada o pagada, bloquea la cancelación."""
        editable_states = ('draft', 'verify')
        for record in self:
            payslip = record.payslip_id
            if payslip and payslip.state not in editable_states:
                raise UserError(_(
                    'No se puede cancelar la novedad %s porque la nómina %s está en estado %s. '
                    'Solo se puede cancelar si la nómina está en borrador.'
                ) % (record.name, payslip.number or payslip.id, payslip.state))
            vals = {'state': 'cancelled'}
            if payslip:
                vals['payslip_id'] = False
                record.message_post(
                    body=_('Novedad cancelada y desvinculada de la nómina %s (borrador). '
                           'Recalcule la nómina para reflejar el cambio.') % (payslip.number or payslip.id),
    ...
    def _notify_approvers(self):
        ...
        partner_ids = approver_group.all_user_ids.mapped('partner_id').ids
...
class HrNoveltyMassiveWizard(models.TransientModel):
    ...
    def _get_employees(self):
        ...
        """
        domain = [('contract_id.state', 'in', ['open', 'finished'])]  # Solo empleados con contrato activo o finalizado
        ...
        if self.selection_type == 'all':
            ...
            return self.employee_ids.filtered(lambda e: e.contract_id.state in ['open', 'finished'])
