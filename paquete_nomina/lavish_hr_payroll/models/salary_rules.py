...
class HrSalaryRule(models.Model):
    ...
    def _anticipo(self, ld):
        ...
        payslip = ld['payslip']
        suma = 0
        # modelo nuevo: advance_line_ids (hr.payslip.advance.line) con amount_discount
        for adv in payslip.advance_line_ids:
            res = True
            suma += adv.amount_discount
        if res:
            lr[0] = suma
        else:
            lr[0] = 'na'
    ...
    def _int_cesantia_anual(self, ld):
        ...
        if res:
            ...
            # Corregir fórmula: debe ser (base_cesantias * 0.12 * dias_trabajados) / 360
            # donde result_qty son los días trabajados del período
            if result_qty > 0:
                lr[0] = result * (0.12 * result_qty) / 360
            else:
                # Si no hay días trabajados registrados, usar fórmula mensual (30 días)
                lr[0] = result * (0.12 * 30) / 360
        else:
            lr[0] = 'na'
        return lr
