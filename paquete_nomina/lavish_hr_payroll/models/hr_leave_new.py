...
class HrHolidays(models.Model):
    ...
    # allocation_rule_id removed: hr.leave.allocation.rule does not exist in Odoo 19
    ...
    vacaciones = fields.Boolean(string='Vacations', compute='_compute_vacaciones')
    ...
    apply_cut = fields.Boolean(string='Apply in Cut?', default=False)
    ...
    general_illness_ext = fields.Boolean(string='Extension General Illness', default=False)
    general_illness = fields.Boolean(string='General Illness', default=False)
    atep = fields.Boolean(string='ATEP', default=False)
    ...
    def _compute_vacaciones(self):
        for leave in self:
            leave.vacaciones = False

    @api.depends('date_from', 'date_to', 'employee_id')
    def _compute_contract(self):
        for leave in self:
            leave.contract_id = False
            if leave.employee_id and leave.date_from:
                date = leave.date_from.date() if hasattr(leave.date_from, 'date') else leave.date_from
    ...
    def _compute_working_hours(self):
        for leave in self:
            if leave.contract_id:
                if leave.vacaciones:
                    leave.working_hours_id = leave.contract_id.resource_calendar_id
                else:
    ...
    def _compute_number_of_days(self):
        for leave in self:
            ...
            # hr.leave.line guarda las horas de nómina en el campo 'hours' (ver _compute_regular_leave),
            # no en 'hours_payslip' (ese vive en hr.holidays.days, modelo paralelo no conectado a line_ids)
    ...
    def write(self, vals):
        # Se permiten los estados de validación (validate/validate1): el flujo estándar de Odoo
        # (_action_validate y los tipos 'no_validation') escribe state='validate' desde 'draft'.
        if 'state' in vals and vals['state'] not in ['draft', 'confirm', 'cancel', 'validate', 'validate1'] and self.filtered(lambda holiday: holiday.state == 'draft'):
