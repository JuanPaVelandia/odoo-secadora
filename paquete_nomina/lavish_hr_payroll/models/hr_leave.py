...
class HolidaysRequest(models.Model):    
    ...
    # Compatibilidad con vistas legacy: flags de acciones usadas en formularios/kanban.
    can_approve = fields.Boolean(compute='_compute_action_flags')
    can_cancel = fields.Boolean(compute='_compute_action_flags')
    can_reset = fields.Boolean(compute='_compute_action_flags')
    ...
    def _compute_action_flags(self):
        """Expose action flags expected by legacy XML views."""
        can_approve = self.env.user.has_group('hr_holidays.group_hr_holidays_user') or self.env.user.has_group('hr_holidays.group_hr_holidays_manager')
        for leave in self:
            leave.can_approve = can_approve
            leave.can_cancel = leave.state not in ('refuse',)
            leave.can_reset = leave.state in ('confirm', 'refuse', 'validate1')
    ...
    force_payroll_value = fields.Boolean(
        string='Forzar Valor Manual',
        tracking=True,
        help='Permite editar manualmente el valor de la ausencia.\n'
             '------------------------------------------------------------\n'
             'COMPORTAMIENTO:\n'
             '  [x] Marcado .... No recalcula payroll_value automaticamente\n'
             '  [ ] Desmarcado . Se recalcula según reglas y líneas\n'
             '------------------------------------------------------------\n'
             'NOTA: Si esta activo, los procesos de recalculo no deben\n'
             'sobrescribir el valor manual.'
    )
    ...
    #####################
    # v19 fix: todos los campos del mismo compute deben compartir store
    # para evitar warning "inconsistent store for computed fields".
    number_of_days_temp = fields.Float(compute='_compute_number_of_days', string='Días de licencia', readonly=True, store=True)
    number_of_days_in_payslip = fields.Float(compute='_compute_number_of_days', string='Días en la nómina', store=True)
    number_of_hours_in_payslip = fields.Float(compute='_compute_number_of_days', string='Horas en nómina', store=True)
    number_of_hours = fields.Float(compute='_compute_number_of_days', string='Horas de licencia', store=True)
    ...
    def _get_hours_per_day(self, date_ref=None, company_id=None):
        ...
        """
        if not date_ref:
            date_ref = self.date_from.date() if hasattr(self.date_from, 'date') else self.date_from
        if not company_id:
    ...
    def _compute_ibl_detalle_html(self):
        ...
        TIPOS DE LIQUIDACION:
            ...
            - YEAR: Usa promedio del ano anterior (vacaciones)
                    ...
                    if payslips:
                        ...
                        detalle_origen += f"""
                            ...
                            <tfoot class='table-light'>
                                <tr><td colspan='2'><strong>Total mes anterior</strong></td><td class='text-end'><strong>${total_base:,.2f}</strong></td></tr>
                                <tr><td colspan='2'><small class='text-muted'>Promedio por nómina ({len(payslips)})</small></td><td class='text-end'><small class='text-muted'>${promedio:,.2f}</small></td></tr>
                            </tfoot>
                        </table>
                        """
                        ibl_usado = leave.ibc or total_base
                    else:
                        detalle_origen = f"""
    ...
    def _compute_duration(self):
        ...
        for holiday in self:
            if holiday.number_of_vac_money_days:
                ...
                days, hours = durations[holiday.id]
    ...
    def action_draft(self):
        """Resetear ausencia a estado inicial (confirm en Odoo 19)"""
        for record in self:
            record._clean_leave()
        self.write({
            'state': 'confirm',
    ...
    def action_reset_confirm(self):
        """Resetear ausencia a confirmado - Odoo 19"""
        for record in self:
            record._clean_leave()
        ...
        for record in self:
            ...
            if contracts and self.date_to:
                ...
                if record.contract_id and record.contract_id.parcial:
                    ...
                    record.ibc = record.contract_id.wage
                if not record.force_payroll_value:
                    if record.line_ids:
    ...
    def _get_ibc_last_month(self, date_to, contract):
        ...
        # Fuente de verdad para mes anterior:
        # consolidado de líneas IBD del mes anterior (quincenas done/paid).
        ...
        ibd_total = sum(ibd_lines.mapped('total'))
        if ibd_total > 0:
    ...
    def compute_line(self):
        # Recalcular IBC del mes anterior al ejecutar "Recalcular" en la ausencia.
        # Sin esto, compute_holiday usa el valor cacheado en leave.ibc.
        for leave in self:
            if not leave.holiday_status_id or not leave.contract_id or not leave.date_to:
                continue
            status = leave.holiday_status_id
            # Obtener la regla asociada al tipo de ausencia (secuencia 1)
            try:
                _, rule_id = status.get_rate_concept_id(1)
                rule_rec = self.env['hr.salary.rule'].browse(rule_id) if rule_id else None
            except Exception:
                rule_rec = None
            # Necesita IBC si el tipo liquida por IBC, o si la regla tiene
            # base_seguridad_social o liquidar_con_base activos
            needs_ibc = (
                status.liquidacion_value == 'IBC'
                or (rule_rec and rule_rec.base_seguridad_social)
                or (rule_rec and rule_rec.liquidar_con_base)
            )
            if needs_ibc and not leave.force_ibc:
                ref_date = leave.date_to.date() if hasattr(leave.date_to, 'date') else leave.date_to
                leave.ibc = leave._get_ibc_last_month(ref_date, leave.contract_id)
    ...
    def _apply_manual_payroll_value(self):
        """Reparte el payroll_value manual en las líneas de ausencia."""
        for leave in self:
            if not leave.force_payroll_value:
                continue
            total = leave.payroll_value or 0.0
            lines = leave.line_ids.filtered(lambda l: not l.is_complement and l.days_payslip)
            if not lines:
                continue
            total_days = sum(lines.mapped('days_payslip'))
            if not total_days:
                continue
            currency = leave.employee_id.company_id.currency_id
            rounding = currency.rounding if currency else 0.01
            remaining = total

            # Anular complementos cuando el valor es manual
            complement_lines = leave.line_ids.filtered(lambda l: l.is_complement)
            if complement_lines:
                complement_lines.write({'amount': 0.0})

            lines_sorted = lines.sorted(lambda l: (l.date or date.min, l.sequence or 0, l.id))
            for idx, line in enumerate(lines_sorted):
                if idx == len(lines_sorted) - 1:
                    amount = remaining
                else:
                    amount = tools.float_round(
                        (total * line.days_payslip) / total_days,
    ...
    def calcular_valor_pago_incapacidad(self, day_data, current_date, sequence, is_rest_day, is_day_31, rule=None):
        """
        ...
        annual_parameters = self.env['hr.annual.parameters'].search([('year', '=', holiday.date_from.year)], limit=1)
        ...
        # Si la regla tiene base_seguridad_social o liquidar_con_base activos,
        # se usa el IBC del mes anterior como base de pago (override del tipo WAGE)
        if rule and not holiday.force_base_amount and not holiday.force_min_wage and not holiday.contract_id.parcial:
            rule_rec = holiday.env['hr.salary.rule'].browse(rule) if isinstance(rule, int) else rule
            use_ibc = bool(rule_rec and (rule_rec.base_seguridad_social or rule_rec.liquidar_con_base))
            if use_ibc:
                ibc_base = holiday.ibc or holiday.ibc_pila or holiday._get_ibc_last_month(holiday.date_to.date() if hasattr(holiday.date_to, 'date') else holiday.date_to, holiday.contract_id) or holiday.contract_id.wage
                pay_amount = ibc_base
                base_type = 'ibc'
    ...
    def _generate_formula_html(self, base_type, ibc_base, rate_applied, amount_real,
        ...
        annual_params = self.env['hr.annual.parameters'].search([('year', '=', self.date_from.year)], limit=1)
        hours_monthly = annual_params.hours_monthly or 240
    ...
    def compute_holiday(self):
        for holiday in self:
            ...
            if not extension_leave and holiday.holiday_status_id.novelty in ('ige', 'irl'):
                ...
                if previous_leave:
                    prev_date_to = previous_leave.date_to.date() if hasattr(previous_leave.date_to, 'date') else previous_leave.date_to
            ...
            if not holiday.force_payroll_value:
            ...
            if (holiday.holiday_status_id.completar_salario and 
                ...
                if diferencia > 0.01:  # Tolerancia para evitar lineas de centavos
                    ...
                    complement_vals = {
                        ...
                        'date': holiday.date_to.date() if hasattr(holiday.date_to, 'date') else holiday.date_to,
                        'hours_assigned': 0,
            ...
            if holiday.force_payroll_value:
                holiday._apply_manual_payroll_value()
    ...
    def _compute_regular_leave(self, holiday, sequence):
        ...
        while current_date <= date_to.date():
            ...
            amount_real, ibc_day, ibc_base, rate_applied, base_type, ibc_original = holiday.calcular_valor_pago_incapacidad(
                day_data, current_date, sequence, is_rest_day, is_day_31, rule=rule
            )
            if is_rest_day and holiday.holiday_status_id.code in ('VACDISFRUTADAS', 'VACVAR'):
                # Mantener trazabilidad pero no pagar ni contar como días de nómina.
                days_payslip = 0
                amount_real = 0
    ...
    def validate_february_days(self, leave):
        ...
        if end_month == 2 and leave.date_to.day == feb_last_day:
            ...
            annual_parameters = self.env['hr.annual.parameters'].search([('year', '=', leave.date_from.year)],limit=1)
            smmlv = annual_parameters.smmlv_monthly
    ...
    def recompute_amounts(self):
        ...
        for holiday in self:
            ...
            _logger.info('Recomputing amounts with base: %s (%s)', amount, base_type)
            ...
            _logger.info('Recomputed amounts for holiday, total: %s', sum(x.amount for x in holiday.line_ids))
            if not holiday.force_payroll_value:
                holiday.payroll_value = sum(x.amount for x in holiday.line_ids)
            else:
                holiday._apply_manual_payroll_value()
    ...
    def write(self, vals):
        ...
        if 'payroll_value' in vals or vals.get('force_payroll_value'):
            self.filtered(lambda l: l.force_payroll_value)._apply_manual_payroll_value()
        return res
...
class HrLeaveLine(models.Model):
    ...
    def unlink(self):
        ...
        for line in self:
            ...
            _logger.info('Eliminando linea de ausencia %s de la ausencia %s', line.id, line.leave_id.name)
    ...
    def sync_holidays_for_year(self, year):
        ...
        try:
            if self._is_year_synced(year):
                _logger.debug('Ano %s ya sincronizado, omitiendo', year)
                return True
            ...
            if created:
                _logger.info('Creados %d festivos para el ano %s', len(created), year)
            else:
                _logger.info('Todos los festivos del ano %s ya existen', year)

            self._mark_year_synced(year)
            return True

        except Exception as e:
            _logger.error('Error sincronizando festivos del ano %s: %s', year, str(e))
            return False
