...
class Hr_payslip(models.Model):
    ...
    def _get_batch_account_setting(self):
        ...
        if param in ('0', '1'):
            return param
        # Fallback to annual parameters configuration
        try:
        ...
        # Default to per-employee if no config is found
        return '1'
    ...
    def _get_default_partner(self, line, slip):
        ...
        return slip.employee_id.work_contact_id or slip.employee_id.user_id.partner_id
            ...
            for slip_date in slip_mapped_data[journal_id]:  # For each month.
                ...
                if by_employee:
                    ...
                    for slip in slips:
                        ...
                        last_slip = slip
                    # Aplicar UNA sola línea de ajuste al peso para el lote completo.
                    # Antes se llamaba dentro del for-slip, acumulando ajustes por slip
                    # y desbalanceando progresivamente -> "asiento no balanceado".
                    if last_slip and line_ids:
                        line_ids = self._apply_adjustment_line(
                            last_slip, line_ids, debit_sum, credit_sum, date, analytic_account_id, precision
                        )
                    if line_ids:
                        move_dict['line_ids'] = [(0, 0, line_vals) for line_vals in line_ids]
                        move = self.env['account.move'].create(move_dict)
