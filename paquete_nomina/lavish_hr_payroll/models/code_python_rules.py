...
if ((aplicar == 0) or (aplicar >= day_initial_payrroll and aplicar <= day_end_payrroll)) or (inherit_contrato!=0):
    if obj_salary_rule:
        ...
        if len(obj_overtime) > 0:
            ...
            # Verificar si cruza julio para aplicar diferentes tarifas
            if payslip.date_from.month == 7 or payslip.date_to.month == 7:
                from datetime import date
                july_15 = date(payslip.date_from.year, 7, 15)
                ...
                for overtime in obj_overtime:
                    if obj_type_overtime.type_overtime == 'overtime_ext_d' and overtime.overtime_ext_d > 0:
                        ...
                        # Obtener configuración de horas para la fecha del overtime
                        hours_config = self.env['hr.company.working.hours'].get_current_hours(
                            company_id=employee.company_id.id, 
                            date=overtime.date
                        )
                        base_hours = hours_config['hours'].hours_per_month if hours_config['hours'] else 240
                        
                        # Calcular valor con la base de horas correcta para esa fecha
                        hourly_rate = contract.wage / base_hours
                        total_value += overtime.overtime_ext_d * hourly_rate * 1.25
                
                result = round(total_value) if r_qty > 0 else 0
            else:
                # Cálculo normal para meses que no cruzan julio
                for overtime in obj_overtime:
                    if obj_type_overtime.type_overtime == 'overtime_ext_d' and overtime.overtime_ext_d > 0:
                        r_qty += overtime.overtime_ext_d
                        
                # Obtener las horas laborales según la Ley 2101 de 2021
                hours_config = payslip.env['hr.company.working.hours'].get_current_hours(
                    company_id=employee.company_id.id, 
                    date=payslip.date_from
                )
                base_hours = hours_config['hours'].hours_per_month if hours_config['hours'] else 240
                result = round((contract.wage / base_hours) * 1.25)  if r_qty > 0 else 0
            ...
            if payslip.date_from.month == 7 or payslip.date_to.month == 7:
                ...
                for overtime in obj_overtime:
                    if obj_type_overtime.type_overtime == 'overtime_eddf' and overtime.overtime_eddf > 0:
                        ...
                        # Calcular valor con la base de horas correcta para esa fecha
                        hourly_rate = contract.wage / base_hours
                        total_value += overtime.overtime_eddf * hourly_rate * 2.0
                ...
                base_hours = hours_config['hours'].hours_per_month if hours_config['hours'] else 240
                result = round((contract.wage / base_hours) * 2)  if r_qty > 0 else 0
            ...
            if payslip.date_from.month == 7 or payslip.date_to.month == 7:
                ...
                for overtime in obj_overtime:
                    if obj_type_overtime.type_overtime == 'overtime_ext_n' and overtime.overtime_ext_n > 0:
                        ...
                        # Calcular valor con la base de horas correcta para esa fecha
                        hourly_rate = contract.wage / base_hours
                        total_value += overtime.overtime_ext_n * hourly_rate * 1.75
                ...
                base_hours = hours_config['hours'].hours_per_month if hours_config['hours'] else 240
                result = round((contract.wage / base_hours) * 1.75)  if r_qty > 0 else 0
            ...
            if payslip.date_from.month == 7 or payslip.date_to.month == 7:
                ...
                for overtime in obj_overtime:
                    if obj_type_overtime.type_overtime == 'overtime_rndf' and overtime.overtime_rndf > 0:
                        ...
                        base_hours = hours_config['hours'].hours_per_month if hours_config['hours'] else 240
                        progressive_surcharge = hours_config['sunday_surcharge']
                        
                        # Calcular valor con la base de horas correcta para esa fecha y el recargo progressivo
                        hourly_rate = contract.wage / base_hours
                        surcharge_multiplier = (100 + progressive_surcharge) / 100
                        total_value += overtime.overtime_rndf * hourly_rate * surcharge_multiplier
                ...
                # Obtener el recargo dominical progresivo para fechas fuera de julio
                hours_config = payslip.env['hr.company.working.hours'].get_current_hours(
                    company_id=employee.company_id.id, 
                    date=payslip.date_from
                )
                ...
                base_hours = hours_config['hours'].hours_per_month if hours_config['hours'] else 240
                surcharge_multiplier = (100 + progressive_surcharge) / 100
                result = round((contract.wage / base_hours) * surcharge_multiplier)  if r_qty > 0 else 0
            ...
            if payslip.date_from.month == 7 or payslip.date_to.month == 7:
                ...
                for overtime in obj_overtime:
                    if obj_type_overtime.type_overtime == 'overtime_rdf' and overtime.overtime_rdf > 0:
                        ...
                        # Calcular valor con la base de horas correcta para esa fecha y el recargo progresivo
                        hourly_rate = contract.wage / base_hours
                        surcharge_multiplier = progressive_surcharge / 100
                        total_value += overtime.overtime_rdf * hourly_rate * surcharge_multiplier
            ...
            if payslip.date_from.month == 7 or payslip.date_to.month == 7:
                ...
                for overtime in obj_overtime:
                    if obj_type_overtime.type_overtime == 'overtime_dof' and overtime.overtime_dof > 0:
                        ...
                        # Calcular valor con la base de horas correcta para esa fecha
                        hourly_rate = contract.wage / base_hours
                        total_value += overtime.overtime_dof * hourly_rate * 1.75
            ...
            if payslip.date_from.month == 7 or payslip.date_to.month == 7:
                ...
                for overtime in obj_overtime:
                    if obj_type_overtime.type_overtime == 'overtime_rn' and overtime.overtime_rn > 0:
                        ...
                        # Calcular valor con la base de horas correcta para esa fecha
                        hourly_rate = contract.wage / base_hours
                        total_value += overtime.overtime_rn * hourly_rate * 0.35
                ...
                base_hours = hours_config['hours'].hours_per_month if hours_config['hours'] else 240
                result = round((contract.wage / base_hours) * 0.35)  if r_qty > 0 else 0
            ...
            if payslip.date_from.month == 7 or payslip.date_to.month == 7:
                ...
                for overtime in obj_overtime:
                    if obj_type_overtime.type_overtime == 'overtime_endf' and overtime.overtime_endf > 0:
                        ...
                        # Calcular valor con la base de horas correcta para esa fecha
                        hourly_rate = contract.wage / base_hours
                        total_value += overtime.overtime_endf * hourly_rate * 2.5
                ...
                base_hours = hours_config['hours'].hours_per_month if hours_config['hours'] else 240
                result = round((contract.wage / base_hours) * 2.5) if r_qty > 0 else 0
...
if ((aplicar == 0) or (aplicar >= day_initial_payrroll and aplicar <= day_end_payrroll)) or (inherit_contrato!=0):
    if obj_salary_rule:
        ...
        if len(obj_overtime) > 0:
            ...
            base_hours = hours_config['hours'].hours_per_month if hours_config['hours'] else 240
            result = round((contract.wage / base_hours) * 0.75)  if r_qty > 0 else 0
            result_qty = r_qty
