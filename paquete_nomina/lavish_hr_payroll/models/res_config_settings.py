...
class ResConfigSettings(models.TransientModel):
    ...
    prst_wo_absences = fields.Boolean(
        'Ignorar ausencias en provisiones',
        default=True,
        help='Si esta activo, las provisiones se calculan sin descontar ausencias ni novedades.'
    )
    ...
    vacaciones_dias_por_ano = fields.Integer(
        'Dias vacaciones por ano',
        default=15,
        help='Dias habiles de vacaciones por ano trabajado: 15'
    )
    ...
    def _setup_structure_sections(self, company):
        ...
        # Buscar todas las estructuras (en Odoo 19 hr.payroll.structure no tiene company_id)
    ...
    def set_values(self):
        ...
        set_param('lavish_hr_payroll.prst_wo_absences', self.prst_wo_absences)
    ...
    def get_values(self):
        ...
        res['prst_wo_absences'] = get_param('lavish_hr_payroll.prst_wo_absences', True)
        return res
        ...
        for entry_data in work_entry_types:
            ...
            if existing_entry:
                if force_update:
                    ...
                    _logger.info(f"Tipo de entrada de trabajo actualizado: {entry_data['name']}")
                else:
                    _logger.info(f"Tipo de entrada de trabajo ya existe (no actualizado): {entry_data['name']}")
            else:
                # Crear nuevo
                new_entry = self.env['hr.work.entry.type'].create(entry_data)
                _logger.info(f"Tipo de entrada de trabajo creado: {entry_data['name']}")
        ...
        for leave_data in leave_types:
            ...
            if existing_leave:
                if force_update:
                    ...
                    _logger.info(f"Tipo de ausencia actualizado: {leave_data['name']}")
                else:
                    _logger.info(f"Tipo de ausencia ya existe (no actualizado): {leave_data['name']}")
            else:
                # Crear nuevo
                new_leave = self.env['hr.leave.type'].create(leave_data)
                _logger.info(f"Tipo de ausencia creado: {leave_data['name']}")
    ...
    def _link_leave_types_to_rules(self):
        ...
        for leave_code, config in LEAVE_RULE_MAPPING.items():
            ...
            try:
                ...
                _logger.info(f"Tipo de ausencia {leave_code} vinculado a reglas salariales")
                updated += 1
            except Exception as e:
                _logger.error(f"Error vinculando {leave_code}: {e}")

        _logger.info(f"Total tipos de ausencia vinculados: {updated}")
        return updated
    ...
    def _get_or_create_structure_type(self):
        ...
        if not structure_type:
            ...
            _logger.info(f"Tipo de estructura salarial creado: {structure_type.name}")
    ...
    def _get_rule_properties(self, code, name, category_code, sequence, process_code='nomina', 
        ...
        base_props = {
            'code': code,
            'name': name,  # Mantener formato original (primera mayuscula)
            'sequence': sequence,
            'active': True,
    ...
    def _generate_all_rules(self):
        ...
        """
        from .rule_generators.index import ALL_GENERATORS
        
        _logger.info("Iniciando generación de reglas salariales...")
        
        total_rules = 0
        for generator_class in ALL_GENERATORS:
            generator_name = generator_class.__name__
            _logger.info(f"Generando reglas de categoría: {generator_name}")
            ...
            try:
                ...
                _logger.info(f"  ✓ {category_count} reglas generadas de {generator_name}")
                
            except Exception as e:
                _logger.error(f"Error generando reglas de {generator_name}: {str(e)}")
                continue
        
        _logger.info(f"Generación completada: {total_rules} reglas salariales creadas/actualizadas")
