...
class CategoryData:
    ...
    # ══════════════════════════════════════════════════════════════════════════
    # Metodos aritmeticos para compatibilidad con reglas salariales
    # Permiten: categories['BASIC'] + categories['ALW'] + categories['DED']
    # ══════════════════════════════════════════════════════════════════════════

    def __float__(self) -> float:
        """Convierte a float retornando el total."""
        return float(self.total)

    def __int__(self) -> int:
        """Convierte a int retornando el total redondeado."""
        return int(self.total)

    def __add__(self, other) -> float:
        """Suma: CategoryData + (number|CategoryData) -> float."""
        if isinstance(other, CategoryData):
            return self.total + other.total
        return self.total + float(other)

    def __radd__(self, other) -> float:
        """Suma reversa: number + CategoryData -> float."""
        return float(other) + self.total

    def __sub__(self, other) -> float:
        """Resta: CategoryData - (number|CategoryData) -> float."""
        if isinstance(other, CategoryData):
            return self.total - other.total
        return self.total - float(other)

    def __rsub__(self, other) -> float:
        """Resta reversa: number - CategoryData -> float."""
        return float(other) - self.total

    def __mul__(self, other) -> float:
        """Multiplicacion: CategoryData * number -> float."""
        return self.total * float(other)

    def __rmul__(self, other) -> float:
        """Multiplicacion reversa: number * CategoryData -> float."""
        return float(other) * self.total

    def __truediv__(self, other) -> float:
        """Division: CategoryData / number -> float."""
        return self.total / float(other)

    def __neg__(self) -> float:
        """Negacion: -CategoryData -> float."""
        return -self.total

    def __pos__(self) -> float:
        """Positivo: +CategoryData -> float."""
        return self.total

    def __abs__(self) -> float:
        """Valor absoluto: abs(CategoryData) -> float."""
        return abs(self.total)

    def __round__(self, ndigits=None) -> float:
        """Redondeo: round(CategoryData, n) -> float."""
        return round(self.total, ndigits)

    def __bool__(self) -> bool:
        """Bool: bool(CategoryData) -> True si total != 0."""
        return self.total != 0.0

    def __eq__(self, other) -> bool:
        """Igualdad: CategoryData == (number|CategoryData)."""
        if isinstance(other, CategoryData):
            return self.total == other.total
        return self.total == other

    def __lt__(self, other) -> bool:
        """Menor que: CategoryData < (number|CategoryData)."""
        if isinstance(other, CategoryData):
            return self.total < other.total
        return self.total < other

    def __le__(self, other) -> bool:
        """Menor o igual: CategoryData <= (number|CategoryData)."""
        if isinstance(other, CategoryData):
            return self.total <= other.total
        return self.total <= other

    def __gt__(self, other) -> bool:
        """Mayor que: CategoryData > (number|CategoryData)."""
        if isinstance(other, CategoryData):
            return self.total > other.total
        return self.total > other

    def __ge__(self, other) -> bool:
        """Mayor o igual: CategoryData >= (number|CategoryData)."""
        if isinstance(other, CategoryData):
            return self.total >= other.total
        return self.total >= other
...
class CategoryCollection:
    ...
    def __getitem__(self, code: str) -> CategoryData:
        """
        Acceso directo por código.
        Retorna CategoryData vacío si no existe (para compatibilidad).
        ...
        # Retornar CategoryData vacío para evitar KeyError
        return CategoryData(
            code=code,
            total=0.0,
            quantity=0.0,
...
class RulesCollection:
    ...
    def add_rule(self, rule_data: RuleData):
        ...
        if rule_data.code in self._rules_by_code:
            ...
            existing.has_leave = existing.has_leave or rule_data.has_leave
            if rule_data.leave_id:
                existing.leave_id = rule_data.leave_id
            if rule_data.leave_novelty:
                existing.leave_novelty = rule_data.leave_novelty
            if rule_data.leave_liquidacion_value:
                existing.leave_liquidacion_value = rule_data.leave_liquidacion_value
        else:
