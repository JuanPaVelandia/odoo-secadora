...
class PayrollFileProcessLog(models.Model):
    ...
    @api.depends('type', 'employee_id', 'employee_id.name', 'name')
    def _compute_display_name(self):
        for log in self:
            type_label = dict(log._fields['type'].selection).get(log.type)
            employee_name = log.employee_id.name or ''
            log_name = log.name or ''
            log.display_name = f"{type_label} - {employee_name} - {log_name}"
...
class HrPayrollFlatFileDetail(models.Model):
    ...
    def _get_account_balance(self, account, date=None):
        ...
        return moves[0][1] or 0.0 if moves else 0.0
...
class HrPayrollFlatFile(models.Model):
    ...
    def generate_flat_file_popular_excel(self,obj_payslip):
        ...
        for payslip in obj_payslip:
            ...
            sheet.write(aument_rows, 4, payslip.employee_id.work_contact_id.first_name if payslip.employee_id.work_contact_id.first_name!=False else '')
