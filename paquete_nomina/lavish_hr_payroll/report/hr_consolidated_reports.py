...
class HrConsolidatedReports(models.TransientModel):
    ...
    def generate_excel_vacaciones(self):
        ...
        query_report = f'''
                ...
                inner join hr_contract as b on a.id = b.employee_id and b.active = true {self.query_filter_period('and','b.date_start')}
                                                and (b.state IN ('open', 'finished') or (({self.query_filter_period('','b.retirement_date',1)} {self.query_filter_period('and','b.retirement_date')}) or ({self.query_filter_period('','b.retirement_date',0,'>=')})))
