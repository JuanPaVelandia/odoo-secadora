...
class HrWithholdingAndIncomeCertificate(models.TransientModel):
    ...
    def generate_certificate(self):
        ...
        obj_annual_parameters = self.env['hr.annual.parameters'].get_for_year(
            self.year, company_id=company_id, raise_if_not_found=False
        )
        for employee in self.employee_ids:
            ...
            if len(obj_annual_parameters) > 0:
                ...
                for conf in sorted(obj_annual_parameters.conf_certificate_income_ids, key=lambda x: (x.sequence, x.id)):
