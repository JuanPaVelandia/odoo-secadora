...
class HrPayslipReportsTemplate(models.Model):
    ...
    @api.depends('type_report', 'company_id', 'company_id.name')
    def _compute_display_name(self):
        for record in self:
            report_type = record.type_report.upper() if record.type_report else ''
            company_name = record.company_id.name if record.company_id else ''
            record.display_name = "Plantilla Reporte {} de {}".format(report_type, company_name)
