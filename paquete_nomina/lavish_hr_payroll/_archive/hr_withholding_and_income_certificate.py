    ...
    def generate_certificate(self):
        ...
        for employee in self.employee_ids:
            if self.save_documents:
                struct_report_income_and_withholdings_finally = ''
            header = self.env['hr.certificate.income.header'].search([
                ('year', '=', self.year),
                ('company_id', '=', employee.company_id.id),
                ('active', '=', True),
            ], limit=1)
            if header:
                struct_report_income_and_withholdings = header.report_template
                struct_report_income_and_withholdings = struct_report_income_and_withholdings.replace(
                    '{year}', str(header.year)
                )
                lst_items = []
                #Obtener nóminas
                year_process = header.year
                year_process_ant = header.year - 1
                # Obtener fechas del periodo seleccionado
                try:
                    date_start = datetime(year_process, 1, 1).date()
                    date_end = datetime(year_process, 12, 31).date()
                    date_start_ant = datetime(year_process_ant, 1, 1).date()
                    date_end_ant = datetime(year_process_ant, 12, 31).date()
                except Exception:
                    raise UserError('El año configurado es inválido, por favor verificar.')
                ...
                for conf in sorted(header.line_ids, key=lambda x: (x.sequence, x.id)):
                    ...
                    if value != None and value != False:
                        value_str = "{:,.2f}".format(value) if type(value) is float else str(value)
                        struct_report_income_and_withholdings = struct_report_income_and_withholdings.replace(
                            '{val' + str(conf.sequence) + '}', value_str
                        )
                    else:
                        struct_report_income_and_withholdings = struct_report_income_and_withholdings.replace(
                            '{val' + str(conf.sequence) + '}', ''
                        )
                ...
                raise UserError(
                    'No hay configuración activa de certificado de ingresos y retenciones '
                    f'para el año {self.year} y la compañía {employee.company_id.name}.'
                )

            # Limpiar vals no calculados (placeholders {valNN}, {valNN_X}, {valNN.X})
            for sequence_val in range(1, 101):
                struct_report_income_and_withholdings_finally = struct_report_income_and_withholdings_finally.replace(
                    '{val' + str(sequence_val) + '}', ''
                )
                for sequence_val_internal in range(1, 10):
                    struct_report_income_and_withholdings_finally = struct_report_income_and_withholdings_finally.replace(
                        '{val' + str(sequence_val) + '_' + str(sequence_val_internal) + '}', ''
                    )
                    struct_report_income_and_withholdings_finally = struct_report_income_and_withholdings_finally.replace(
                        '{val' + str(sequence_val) + '.' + str(sequence_val_internal) + '}', ''
                    )
                ...
                if self.save_documents:
                    ...
                    pdf_content, dummy = self.env['ir.actions.report'].sudo()._render_qweb_pdf(report, obj_report.id)
