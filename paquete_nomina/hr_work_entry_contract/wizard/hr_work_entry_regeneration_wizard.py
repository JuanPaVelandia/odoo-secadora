...
class HrWorkEntryRegenerationWizard(models.TransientModel):
    ...
    def _uses_legacy_work_entry_datetimes(self):
        work_entry_model = self.env['hr.work.entry']
        return 'date_start' in work_entry_model._fields and 'date_stop' in work_entry_model._fields
    ...
    def _compute_validated_work_entry_ids(self):
        for wizard in self:
            ...
            if wizard.search_criteria_completed:
                search_domain = [('employee_id', 'in', wizard.employee_ids.ids), ('state', '=', 'validated')]
                order = 'date_start'
                if wizard._uses_legacy_work_entry_datetimes():
                    search_domain += [('date_start', '>=', wizard.date_from), ('date_stop', '<=', wizard.date_to)]
                else:
                    search_domain += [('date', '>=', wizard.date_from), ('date', '<=', wizard.date_to)]
                    order = 'date'
                validated_work_entry_ids = self.env['hr.work.entry'].search(search_domain, order=order)
    ...
    def regenerate_work_entries(self):
        ...
        if self._uses_legacy_work_entry_datetimes():
            search_domain += [('date_stop', '>=', date_from), ('date_start', '<=', date_to)]
        else:
            search_domain += [('date', '>=', date_from), ('date', '<=', date_to)]
        work_entries = self.env['hr.work.entry'].search(search_domain)
