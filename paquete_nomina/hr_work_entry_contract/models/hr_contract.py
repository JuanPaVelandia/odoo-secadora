...
class HrContract(models.Model):
    ...
    def _get_interval_leave_work_entry_type(self, interval, leaves, bypassing_codes):
        ...
        leave_type = self.env.ref('hr_work_entry_contract.work_entry_type_leave', raise_if_not_found=False)
        if leave_type:
            return leave_type
        leave_type = self.env['hr.work.entry.type'].search([('is_leave', '=', True)], order='id', limit=1)
        return leave_type
    ...
    def _get_contract_work_entries_values(self, date_start, date_stop):
        if not self._uses_legacy_work_entry_datetimes():
            return []
        ...
        for contract in self:
            ...
            for interval in real_attendances:
                ...
                contract_vals += [dict([
                    ...
                    ('state', 'draft'),
                ] + (contract._get_more_vals_attendance_interval(interval) or []))]
            ...
            for interval in real_leaves:
                ...
                for leave_interval in [(l[0], l[1], interval[2]) for l in leaves_over_interval]:
                    ...
                    contract_vals += [dict([
                        ...
                        ('state', 'draft'),
                        ('contract_id', contract.id),
                    ] + (contract._get_more_vals_leave_interval(interval, interval_leaves) or []))]
    ...
    def _remove_work_entries(self):
        ...
        if not self._uses_legacy_work_entry_datetimes():
            return super()._remove_work_entries()
    ...
    def _cancel_work_entries(self):
        if not self._uses_legacy_work_entry_datetimes():
            return super()._cancel_work_entries()
        if not self:
            return
    ...
    def _cron_generate_missing_work_entries(self):
        if not self._uses_legacy_work_entry_datetimes():
            return super()._cron_generate_missing_work_entries()
