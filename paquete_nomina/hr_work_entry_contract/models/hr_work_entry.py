...
class HrWorkEntry(models.Model):
    ...
    def _uses_legacy_datetime_fields(self):
        return 'date_start' in self._fields and 'date_stop' in self._fields

    def _get_work_entry_table_temporal_columns(self):
        self.env.cr.execute("""
            SELECT column_name
            FROM information_schema.columns
            WHERE table_name = 'hr_work_entry'
        """)
        columns = {row[0] for row in self.env.cr.fetchall()}
        start_column = next((column for column in ('date_start', 'date_from', 'date_begin') if column in columns), None)
        stop_column = next((column for column in ('date_stop', 'date_to', 'date_end') if column in columns), None)
        if start_column and stop_column:
            return 'interval', start_column, stop_column
        if 'date' in columns:
            return 'date', 'date', None
        return None, None, None

    def init(self):
        temporal_mode, start_column, stop_column = self._get_work_entry_table_temporal_columns()
        if temporal_mode == 'interval':
            ...
            self.env.cr.execute(f"""
                ...
                ON hr_work_entry(contract_id, {start_column}, {stop_column})
                WHERE state in ('draft', 'validated');
            """)
        elif temporal_mode == 'date':
            self.env.cr.execute(f"""
                CREATE INDEX IF NOT EXISTS hr_work_entry_contract_date_start_stop_idx
                ON hr_work_entry(contract_id, {start_column})
                WHERE state in ('draft', 'validated');
            """)
        else:
            return
    ...
    def _init_column(self, column_name):
        if column_name != 'contract_id':
                ...
                self.env.cr.execute(f"""
                    ...
                    FROM (
                        ...
                        ON
                            ...
                            hwe.{start_column} >= hc.date_start AND
                            hwe.{stop_column} < COALESCE(hc.date_end + integer '1', '9999-12-31 23:59:59')
                        WHERE
                ...
                self.env.cr.execute(f"""
                    ...
                    FROM (
                        ...
                        ON
                            ...
                            hc.state in ('open', 'close') AND
                            hwe.{start_column} >= hc.date_start AND
                            hwe.{start_column} <= COALESCE(hc.date_end, '9999-12-31')
                        WHERE
                            hwe.contract_id IS NULL
                        GROUP BY
                            hwe.employee_id, hc.id
            ...
            if self._uses_legacy_datetime_fields():
                search_domain += [('date_start', '!=', False), ('date_stop', '!=', False)]
            elif 'date' in self._fields:
                search_domain += [('date', '!=', False)]
            else:
                return

            work_entries = self.with_context(active_test=False).search(search_domain)
            for work_entry in work_entries:
                vals = {'employee_id': work_entry.employee_id.id}
                if work_entry._uses_legacy_datetime_fields():
                    vals.update({
                        'date_start': work_entry.date_start,
                        'date_stop': work_entry.date_stop,
                    })
                else:
                    vals['date'] = work_entry.date
                vals = work_entry._set_current_contract(vals)
                if vals.get('contract_id'):
                    work_entry.contract_id = vals['contract_id']

    def _get_duration_is_valid(self):
        return self.work_entry_type_id and self.work_entry_type_id.is_leave

    @api.onchange('employee_id', 'date_start', 'date_stop', 'date')
    ...
    def _compute_date_stop(self):
        if not self._uses_legacy_datetime_fields():
            return super()._compute_date_stop()
    ...
    def _get_duration_batch(self):
        if not self._uses_legacy_datetime_fields():
            return super()._get_duration_batch()
    ...
    def _set_current_contract(self, vals):
        ...
        if not vals.get('contract_id') and vals.get('date') and vals.get('employee_id'):
            work_date = fields.Date.to_date(vals.get('date'))
            employee = self.env['hr.employee'].browse(vals.get('employee_id'))
            contracts = employee._get_contracts(work_date, work_date, states=['open', 'pending', 'close'])
            if not contracts:
                raise ValidationError(_(
                ...
                raise ValidationError(_(
                    "%(employee)s has multiple contracts on %(date)s. A work entry cannot overlap multiple contracts.",
                    employee=employee.name,
                    date=work_date,
                ))
            return dict(vals, contract_id=contracts[0].id)
