...
class HrPayrollSocialSecurity(models.Model):
    ...
    def get_plano(self):
        ...
        annual_parameters = self.env['hr.annual.parameters'].search([('year', '=', self.year)], limit=1)
        ...
        for item in details:
            ...
            if cTipIdTercero == '/':
                raise ValidationError(_('El tipo de documento del empleado '+item.employee_id.name+' es invalido, '+item.employee_id.work_contact_id.l10n_latam_identification_type_id.dian_code+' por favor verificar.'))           
            #if item.employee_id.permit_no:
            #    cNumIdTercero = left(item.employee_id.permit_no+16*' ',16)
            #else:            
            if not item.employee_id.work_contact_id.vat_co:
                raise ValidationError(_('El empleado '+item.employee_id.name+' no tiene Identificacion.'))    
            ...
            cPrimerNombre = left(cPrimerNombre+' '*20,20) if not item.employee_id.work_contact_id.first_name else left(self.remove_accents(item.employee_id.work_contact_id.first_name) +' '*20,20)
