...
class HrConsolidatedProvisions(models.Model):
    ...
    _consolidated_provisions_uniq = models.Constraint('unique(company_id,year,provision)',
                                                      'El año seleccionado ya esta registrado para esta provisión y compañía, por favor verificar.')

    @api.depends('year', 'provision')
    def _compute_display_name(self):
        for record in self:
            provision_str = dict(self._fields['provision'].selection).get(record.provision)
            record.display_name = "Consolidado {} - {}".format(str(record.year), provision_str or '')
    ...
    def unlink(self):
        ...
        return super(HrConsolidatedProvisions, self).unlink()

class HrConsolidatedProvisionsDetail(models.Model):
