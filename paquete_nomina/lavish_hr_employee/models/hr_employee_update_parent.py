...
class HrEmployee(models.Model):
    ...
    def write(self, vals):
        ...
        if vals.get('parent_id',False):
            ...
            field_name = name_selection_groups(self.env['res.groups'].search([('category_id', '=', user_types_category.id)]).ids) if user_types_category else False
            #Verificar tipo de usuario del Gerente
            if obj_employee_parent.user_id and obj_employee_parent.user_id.has_group('base.group_user'): # Usuario Interno
                user_type = self.env.ref('base.group_user', raise_if_not_found=False).id
            elif obj_employee_parent.user_id and obj_employee_parent.user_id.has_group('base.group_public'): # Usuario Público
                user_type = self.env.ref('base.group_public', raise_if_not_found=False).id
            elif obj_employee_parent.user_id and obj_employee_parent.user_id.has_group('base.group_portal'): # Usuario de Portal
                user_type = self.env.ref('base.group_portal', raise_if_not_found=False).id
            else:
                user_type = False

        obj_write = super(HrEmployee, self).write(vals)
