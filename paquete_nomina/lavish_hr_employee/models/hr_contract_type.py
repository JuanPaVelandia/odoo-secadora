...
- Otros conceptos que constituyan salario'''
    ...
    def get_max_end_date(self, start_date):
        """Calcula la fecha máxima de finalización según la configuración."""
        self.ensure_one()
        if not start_date:
            return False

        if self.max_duration_months > 0:
            return start_date + relativedelta(months=self.max_duration_months)
        elif self.max_duration_days > 0:
            return start_date + relativedelta(days=self.max_duration_days)
        return False

    def check_renewal_allowed(self, current_renewals, total_duration_months):
        """
        Verifica si se permite una renovación adicional.

        Args:
            current_renewals: Número de renovaciones actuales
            total_duration_months: Duración total acumulada en meses

        Returns:
            tuple: (permitido: bool, mensaje: str)
        """
        self.ensure_one()

        # Verificar número de renovaciones
        if self.max_renewals > 0 and current_renewals >= self.max_renewals:
            return False, _(
                'Se ha alcanzado el número máximo de renovaciones (%s). '
                'Si no se notifica con 30 días de anticipación, el contrato '
                'se renovará automáticamente.'
            ) % self.max_renewals

        # Verificar duración total
        if self.max_duration_months > 0 and total_duration_months >= self.max_duration_months:
            return False, _(
                'Se ha alcanzado la duración máxima del contrato (%s meses). '
                'Según la Ley 2466/2025, el contrato debe convertirse a término indefinido.'
            ) % self.max_duration_months

        return True, ''

    def get_apprentice_wage_percentage(self, stage):
        """
        Obtiene el porcentaje de salario para aprendices según la etapa.

        Args:
            stage: 'lectiva' o 'productiva'

        Returns:
            float: Porcentaje del SMMLV a aplicar
        """
        self.ensure_one()
        if not self.is_apprenticeship:
            return 100.0

        if stage == 'lectiva':
            return self.apprentice_wage_pct_lectiva
        elif stage == 'productiva':
            return self.apprentice_wage_pct_productiva
        return 100.0
    ...
    def get_excluded_rule_codes(self):
        """
        Retorna solo los códigos de reglas que deben excluirse.

        Útil para filtrar reglas en la nómina.

        Returns:
            list: Lista de códigos de reglas a excluir
        """
        self.ensure_one()
        result = self.get_applicable_salary_rules()
        return result['excluded_codes']

    def should_calculate_rule(self, rule_code):
        """
        Determina si una regla específica debe calcularse para este tipo de contrato.

        Args:
            rule_code: Código de la regla salarial (ej: 'PRV_PRIM', 'SS_SALUD_EMP')

        Returns:
            bool: True si la regla debe calcularse, False si debe omitirse
        """
        self.ensure_one()

        # Mapeo inverso: código de regla -> campo de beneficio
        code_to_benefit = {}

        for benefit_key, rules_config in self.PRESTACIONES_RULES_MAP.items():
            for rule_type, code in rules_config.items():
                if rule_type != 'base_field':
                    code_to_benefit[code] = benefit_key

        for ss_key, rules_config in self.SEGURIDAD_SOCIAL_RULES_MAP.items():
            for rule_type, code in rules_config.items():
                code_to_benefit[code] = ss_key

        # Si el código no está en el mapeo, permitir por defecto
        if rule_code not in code_to_benefit:
            return True

        # Obtener el beneficio correspondiente
        benefit_key = code_to_benefit[rule_code]
        benefits = self.get_applicable_benefits()

        return benefits.get(benefit_key, True)

    def load_contract_concepts(self, contract):
        """
        Carga los conceptos (devengos/deducciones) al contrato según el tipo.

        Este método crea automáticamente los conceptos recurrentes que aplican
        según la configuración del tipo de contrato.

        Args:
            contract: Registro hr.contract donde cargar los conceptos

        Returns:
            dict: {
                'created': Lista de conceptos creados,
                'skipped': Lista de conceptos omitidos (ya existían),
        ...
        # Definir conceptos a crear según prestaciones
        concepts_to_create = []

        # Auxilio de transporte
        if benefits.get('auxilio_transporte'):
            rule = SalaryRule.search([('code', '=', 'AUX000')], limit=1)
            if rule:
        ...
        # Para cada concepto, verificar si ya existe y crear si no
        for concept_data in concepts_to_create:
            existing = ConceptModel.search([
                ('contract_id', '=', contract.id),
                ('input_id', '=', concept_data['input_id']),
                ('state', '!=', 'cancel'),
            ], limit=1)

            if existing:
                skipped.append({
                    'rule_id': concept_data['input_id'],
                    'reason': 'Ya existe'
                })
                continue

            try:
                concept_data['contract_id'] = contract.id
                new_concept = ConceptModel.create(concept_data)
                created.append(new_concept.id)
            except Exception as e:
                errors.append({
                    'rule_id': concept_data.get('input_id'),
                    'error': str(e)
                })
    ...
    def get_novelty_rules(self):
        """
        Retorna las reglas salariales de tipo novedad aplicables a este tipo de contrato.

        Returns:
            recordset: hr.salary.rule records de tipo novedad
        """
        self.ensure_one()
        SalaryRule = self.env['hr.salary.rule']

        # Buscar reglas de tipo novedad
        domain = [
            ('type_concepts', '=', 'novedad'),
            ('active', '=', True),
        ]

        # Filtrar por tipo de empleado si aplica
        # Las reglas pueden tener restricción por tipo de empleado

        return SalaryRule.search(domain)

    def get_provision_config(self):
        """
        Retorna la configuración de provisiones aplicables.

        Returns:
            dict: {
                'prima': {'active': bool, 'rate': float, 'code': str},
                'cesantías': {...},
                'intereses': {...},
                'vacaciones': {...}
            }
        ...
        # Obtener parámetros anuales para las tasas
        ...
        config = {
            'prima': {
                'active': benefits.get('prima', True),
                'rate': annual_params.value_porc_provision_bonus if annual_params else 8.33,
                'code': 'PRV_PRIM',
            },
            'cesantias': {
                'active': benefits.get('cesantias', True),
                'rate': annual_params.value_porc_provision_cesantias if annual_params else 8.33,
                'code': 'PRV_CES',
            },
            'intereses': {
                'active': benefits.get('intereses_cesantias', True),
                'rate': annual_params.value_porc_provision_intcesantias if annual_params else 12.0,
                'code': 'PRV_ICES',
                'depends_on': 'PRV_CES',
            },
            'vacaciones': {
                'active': benefits.get('vacaciones', True),
                'rate': annual_params.value_porc_provision_vacation if annual_params else 4.17,
                'code': 'PRV_VAC',
            },
        }
    ...
    def validate_contract_benefits(self, contract):
        """
        Valida que el contrato tenga configurados correctamente los beneficios
        según el tipo de contrato.

        Args:
            contract: Registro hr.contract a validar
        ...
        if benefits.get('auxilio_transporte'):
            if contract.not_pay_auxtransportation:
                warnings.append(_(
                    'El tipo de contrato tiene derecho a auxilio de transporte, '
                    'pero el contrato tiene marcado "No liquidar auxilio de transporte".'
                ))

        # Validar pensión para aprendices (Ley 2466)
        if self.is_apprenticeship and not benefits.get('pension'):
            errors.append(_(
                'Los aprendices deben tener derecho a pensión según la Ley 2466/2025.'
            ))

        # Validar contratos ocasionales
        if self.contract_category == 'ocasional':
            if benefits.get('prima') or benefits.get('cesantias'):
                warnings.append(_(
                    'Los contratos ocasionales típicamente no tienen derecho a '
                    'prestaciones sociales (prima, cesantías). Verifique la configuración.'
                ))
