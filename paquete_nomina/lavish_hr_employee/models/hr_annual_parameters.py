...
class HrAnnualParameters(models.Model):
    ...
    provision_quincenal_mode = fields.Selection(
        [
            ('second_only', 'Solo segunda quincena (todo el mes)'),
            ('split', 'Distribuir en cada quincena'),
        ],
        string='Modo provisión quincenal (simple)',
        default='second_only',
        help='Define si las provisiones simples se calculan solo en la segunda quincena '
             'o se distribuyen en cada quincena.'
    )
    ...
    aux_in_variables_liquidation = fields.Boolean(
        'Incluir auxilio en variables (Liquidación)',
        default=False,
        help='Permite que conceptos de categoría AUX se incluyan como variables en liquidaciones '
             'de prestaciones (prima/cesantías). Por defecto, AUX se excluye.'
    )
    ...
    def init(self):
        ...
        if not column_exists(self.env.cr, self._table, 'company_id'):
            return
    ...
    def get_for_year(self, year, company_id=None, raise_if_not_found=True):
        ...
        - Preferencia por registros que incluyan directamente la compañía.
...
class HrCompanyWorkingHours(models.Model):
    ...
    def get_current_hours(self, company_id=None, date=None):
        ...
        # Determinar el recargo dominical según la gradualidad
        if current_year < 2025 or (current_year == 2025 and current_month < 7):
            sunday_surcharge = 75.0
        elif (current_year == 2025 and current_month >= 7) or \
            (current_year == 2026 and current_month < 7):
            sunday_surcharge = 80.0
        elif (current_year == 2026 and current_month >= 7) or \
            (current_year == 2027 and current_month < 7):
            sunday_surcharge = 90.0
        else:
            sunday_surcharge = 100.0
        
        if not hours:
            # Determinar las horas según la Ley 2101
            if current_year < 2023 or (current_year == 2023 and current_month < 7):
                max_hours = 48.0
                hours_to_pay = 240.0
            elif (current_year == 2023 and current_month >= 7) or \
                (current_year == 2024 and (current_month < 7 or (current_month == 7 and current_day < 15))):
                max_hours = 47.0
                hours_to_pay = 235.0
            elif (current_year == 2024 and (current_month > 7 or (current_month == 7 and current_day >= 15))) or \
                (current_year == 2025 and (current_month < 7 or (current_month == 7 and current_day < 15))):
                max_hours = 46.0
                hours_to_pay = 230.0
            elif (current_year == 2025 and (current_month > 7 or (current_month == 7 and current_day >= 15))) or \
                (current_year == 2026 and (current_month < 7 or (current_month == 7 and current_day < 15))):
                max_hours = 44.0
                hours_to_pay = 220.0
            else:
                max_hours = 42.0
            ...
            hours = self.create({
                ...
                'hours_to_pay': hours_to_pay,
                'effective_date': date,
                'notes': f'Ley 2101 - {max_hours}h/semana - Recargo dominical {sunday_surcharge}%'
            })
