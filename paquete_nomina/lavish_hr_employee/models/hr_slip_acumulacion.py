...
class HrPayslipAccumulation(models.AbstractModel):
    ...
    def _get_categories_accumulated(self, start_date, end_date, contract_ids=None):
        ...
        # Query SQL optimizada - INCLUYE información de reglas y ausencias para CategoryCollection
        # Estados Odoo 19: draft, verify, done, cancel, paid
        # RETORNA: Datos completos para construir CategoryCollection con info de ausencias
            query = """
                SELECT
                    HP.contract_id,
                    ...
                    SUM(HPL.quantity) AS quantity,
                    AVG(HPL.rate) AS rate,
                ...
                INNER JOIN hr_salary_rule_category AS HSRC
                    ON HSRC.id = HSR.category_id
                LEFT JOIN hr_leave AS HL
                    ON HL.id = HPL.leave_id
                LEFT JOIN hr_leave_type AS HLT
                ...
                GROUP BY HP.contract_id, HSRC.code, HSR.code, HSR.id
                ORDER BY HP.contract_id, HSRC.code, HSR.code
            """

        params = (
            start_date,
            end_date,
            tuple(contract_ids) if contract_ids else (0,),
            tuple(self.ids) if self.ids else (0,),
            tuple(category_ids) if category_ids else (0,)
        )
    ...
    def _get_concepts_accumulated(self, start_date, end_date, contract_ids=None, concept_codes=None):
        ...
        # Consulta SQL optimizada - ADAPTADA A CAMPOS REALES
        # RETORNA: totales + IDs de líneas de nómina
        query = """
            SELECT
                HP.contract_id,
        ...
        _logger.info(
            ...
            f"{len(concept_codes)} conceptos, período {start_date} - {end_date}"
        )
    ...
    def _get_worked_days_processed(self, period_id=None, contract_id=None):
        ...
        query = """
            ...
            WHERE
                HP.period_id = %s AND
                HP.contract_id = %s AND
                HP.id != %s AND
                HP.state IN ('done', 'paid')
        """

        params = (period_id, contract_id, self.id)
    ...
    def _get_leave_days_accumulated(self, start_date, end_date, contract_id=None, leave_type_codes=None):
        ...
        # Construir filtro de tipos de ausencia
        leave_type_filter = ""
        params = [start_date, end_date, contract_id]

        if leave_type_codes:
            leave_type_filter = "AND HLT.code IN %s"
            params.append(tuple(leave_type_codes))

        # Consulta SQL ADAPTADA - usando estados reales
        # RETORNA: días + montos + IDs de líneas de ausencia
        query = f"""
            SELECT
            ...
            INNER JOIN hr_leave_type AS HLT
                ON HLT.id = HL.holiday_status_id
            WHERE
                HLL.date BETWEEN %s AND %s AND
                HL.contract_id = %s AND
    ...
    def _get_leave_days_no_pay(self, start_date, end_date, contract_id=None):
        ...
        params = (contract_id, start_date, end_date)
        self._cr.execute(query, params)
        result = self._cr.fetchone()

        days_no_pay = result[0] if result else 0
        ...
        query = """
            ...
            FROM (
                ...
                WHERE
                    ...
                    HLL.date BETWEEN %s AND %s AND
                    HLT.code IN ('INC', 'INC_ARL', 'VAC', 'LIC_PAG') AND
    ...
    def _get_overtime_accumulated(self, start_date, end_date, fecha_fin=None, contract_id=None, employee_id=None):
        ...
        SIMPLIFICADO para agrupar por fecha y evitar problemas con horarios.
        ...
        # Query simplificada: agrupa por fecha para evitar problemas con horarios
        query = """
            SELECT
                date,
                SUM(overtime_rn) AS rn,
                SUM(overtime_ext_d) AS ext_d,
                SUM(overtime_ext_n) AS ext_n,
                SUM(overtime_eddf) AS eddf,
                SUM(overtime_endf) AS endf,
                SUM(overtime_dof) AS dof,
                SUM(overtime_rndf) AS rndf,
                SUM(overtime_rdf) AS rdf,
                SUM(overtime_rnf) AS rnf,
                ARRAY_AGG(id) AS overtime_ids
            FROM hr_overtime
            WHERE
                employee_id = %s
                AND date >= %s
                AND date <= %s
                AND state IN ('validated', 'paid', 'done')
            GROUP BY date
            ORDER BY date
        """

        try:
            self._cr.execute(query, (employee_id, start_date, fecha_fin))
            results = self._cr.fetchall()
            ...
            for row in results:
                ...
                # Filtrar tipos con horas
                totals_filtered = {k: v for k, v in totals.items() if v > 0}
                period_hours = sum(totals_filtered.values())
                period_ids = row[10] or []
                ...
                if period_hours > 0:
                    ...
                    all_ids.extend(period_ids)

                    # Acumular totales generales para compatibilidad
                    for tipo, valor in totals_filtered.items():
                        totals_general[tipo] = totals_general.get(tipo, 0) + valor

            _logger.info(
                f"[OVERTIME] Empleado {employee_id}: {len(by_period)} fechas, "
                f"{total_hours} horas totales"
            )
    ...
    def _get_category_with_children(self, category_code):
        ...
        _logger.debug(
            f"Categoría '{category_code}' con jerarquía: {len(all_ids)} categorías"
        )
    ...
    def _get_novelties_accumulated(self, start_date, end_date, contract_id=None, rule_codes=None):
        ...
        Args:
            ...
            end_date (date): Fecha final
            contract_id (int): ID del contrato (opcional)
            rule_codes (list): Códigos de reglas salariales (opcional)
        ...
        # Consulta SQL para novedades
        # RETORNA: montos + cantidades + IDs de novedades
        query = f"""
            SELECT
                HSR.code AS rule_code,
            ...
            INNER JOIN hr_salary_rule AS HSR
                ON HSR.id = HND.salary_rule_id
            WHERE
                HND.employee_id = %s AND
                HND.date BETWEEN %s AND %s AND
        ...
        accumulated = {}
        for rule_code, total_amount, novelty_count, novelty_ids in results:
            accumulated[rule_code] = {
                'amount': total_amount,
                'count': novelty_count,
                'novelty_ids': novelty_ids or []
            }

        _logger.info(
            f"Novedades acumuladas: {len(accumulated)} reglas, "
            f"período {start_date} - {end_date}"
        )
    ...
    def _mark_novelties_as_processed(self, payslip_id, novelty_ids):
        ...
        Args:
            ...
            novelty_ids (list): IDs de novedades a marcar
        """
        if not novelty_ids:
            return
        ...
        _logger.info(f"Marcadas {len(novelty_ids)} novedades como procesadas en nómina {payslip_id}")
    ...
    def get_novelty_ids(self, rule_code):
        ...
        Returns:
            list: IDs de hr.novelties.different.concepts
        """
        self.ensure_one()
        start_date, end_date = self._get_period_range_for_accumulation()
        accumulated = self._get_novelties_accumulated(start_date, end_date, rule_codes=[rule_code])
