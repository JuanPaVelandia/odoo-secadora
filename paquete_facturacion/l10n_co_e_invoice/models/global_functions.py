...
def get_template_xml(values, template_name):
    """
    Genera un XML a partir de una plantilla y un conjunto de valores.

    La plantilla se carga desde un directorio 'templates' en la misma ubicación que el script actual. Los valores proporcionados se utilizan para llenar la plantilla.

    Args:
        values (dict): Un diccionario con los valores para llenar la plantilla.
        template_name (str): El nombre de la plantilla a cargar.

    Returns:
        str: El XML generado a partir de la plantilla y los valores proporcionados. Todos los caracteres '&' en el XML se reemplazan por '&amp;', excepto cuando ya forman parte de '&amp;amp;'.
    """
...
def get_xml_with_signature(
    ...
    # CAMBIO IMPORTANTE: Usar SHA256 en lugar de SHA512
    signature = xmlsig.template.create(
        xmlsig.constants.TransformInclC14N,
        xmlsig.constants.TransformRsaSha256,  # Cambiado de TransformRsaSha512
        signature_id)
    
    ref = xmlsig.template.add_reference(
        signature,
        xmlsig.constants.TransformSha256,  # Cambiado de TransformSha512
        uri="",
    ...
    props = template.create_signed_properties(
        ...
        datetime=date_now_utc)
    
    template.add_claimed_role(props, "supplier")
    
    # Política de firma con SHA256
...
def get_xml_soap_values(certificate_file, certificate_password):
    Created = datetime.now().replace(tzinfo=pytz.timezone('UTC'))
    Created = Created.astimezone(pytz.timezone('UTC'))
