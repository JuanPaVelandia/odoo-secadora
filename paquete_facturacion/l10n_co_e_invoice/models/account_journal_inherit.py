...
class AccountJournalInherit(models.Model):
	...
	default_payment_method_dian_id = fields.Many2one(
		'account.payment.method.dian', string='Forma de Pago Predeterminada',
		help="Forma de pago predeterminada para los documentos de este diario. "
	)

	dian_email_enabled = fields.Boolean(
		string="Enviar Email DIAN",
		default=True,
		help="Si está activo, se enviará email automáticamente cuando el documento sea exitoso en DIAN"
	)

	dian_validate_on_confirm = fields.Boolean(
		string="Validar DIAN al Confirmar",
		default=True,
		help="Si está activo, se enviará automáticamente a DIAN al confirmar la factura"
	)
