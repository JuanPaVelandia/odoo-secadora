...
class SaleOrder(models.Model):
    ...
    payment_request_ids = fields.Many2many(
        ...
        help='Solicitudes de pago/anticipo asociadas a este pedido'
    )
    ...
    def action_create_payment_request(self):
        """Crear solicitud de pago/anticipo desde pedido de venta"""
        self.ensure_one()

        if not self.partner_id:
            raise UserError(_('Debe tener un cliente asignado'))

        # Buscar tipo de anticipo por defecto para ventas
        ...
        if not advance_type:
            raise UserError(_(
                'No se encontró un tipo de anticipo configurado para pedidos de venta.\n'
                'Configure un tipo de anticipo con "Permite Pedidos de Venta" activado.'
            ))
    ...
    def action_add_to_payment_request(self):
        """Agregar este pedido a una solicitud existente"""
        self.ensure_one()

        return {
            'name': _('Agregar a Solicitud'),
            'type': 'ir.actions.act_window',
            'res_model': 'sale.order.add.to.request.wizard',
            'view_mode': 'form',
            'target': 'new',
            'context': {
