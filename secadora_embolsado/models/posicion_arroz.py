...
class PosicionArroz(models.Model):
    ...
    def _objetivos_doble_descuento(self):
        ...
        i_obj = float(Param.get_param('secadora_embolsado.impureza_objetivo', '0.0'))
        return h_obj, i_obj
    ...
    def _estimar_peso_seco(self):
        ...
        posiciones del tablero. Sin humedad registrada se devuelve el peso verde.
        """
        self.ensure_one()
        if not self.humedad:
            return self.peso_kg
        h_obj, i_obj = self._objetivos_doble_descuento()
        factor_h = min((100.0 - self.humedad) / (100.0 - h_obj), 1.0) if h_obj < 100 else 1.0
        factor_i = 1.0
        if self.impurezas and i_obj < 100:
